...
'''
 TOP_HAT - difference between the input image and its opening of the image
 top_hat = cv2.morphologyEx(image, cv2.MORPH_TOPHAT, kernel)
 BACK_HAT - difference between the input image and the closing of the image.
 back_hat = cv.morphologyEx(image, cv2.MORPH_BLACKHAT,kernel)
...
def build_kernel(kernel_type, kernel_size):
    if kernel_type == cv2.MORPH_ELLIPSE:
        ...
        return cv2.getStructuringElement(cv2.MORPH_ELLIPSE, kernel_size) #outputs the desired kernel. takes shape and size
...
#erode - makes the boundary small , holes will become bigger
...
#dilates - makes the boundry large, holes will become smaller
...
# creates small group of undesirable pixels bigger
...
# eliminate small group of undesirable pixels
...
# difference betweem tje dilation and erosion of the image
